...
void main() {
    ...
    vec3 diffuse = kd * diff * texture(texBuff, TexCoord).rgb * lightColor;
...
bool loadModel(const string& path, Modelo& modelo) {
    vector<Vertex> verts;
    if (!loadOBJWithMTL(path, "../assets/Modelos3D/final", verts, modelo.textura)) return false;
...
int main() {
    ...
    while (!glfwWindowShouldClose(w)) {
        ...
        if (casaLuz) {
            ...
            if (vacaY > 0.0f)
                vacaY -= deltaTime * 2.0f;
        } else {
            if (subindo || ovniY > 5.5f) {
                ovniY -= deltaTime * 2.5f;
                descendo = true;
            } else {
                ovniY = 5.0f + sin(t * 2.0f) * 0.5f;
                descendo = false;
            }

            if (vacaY < alturaAbducao)
                vacaY += deltaTime * 1.0f;
        }
        ...
        vec3 lightPos = casaLuz ? vec3(5, 2, -5) : vec3(0, ovniY - 1.0f, 0);
        vec3 lightColor = casaLuz ? vec3(1.0f) : vec3(0.0f, 1.0f, 0.0f);
